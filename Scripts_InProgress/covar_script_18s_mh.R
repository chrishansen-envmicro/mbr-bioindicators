...
indval_filter <- function(in_data, target_data, cutoff = 0.05){
  ...
  dummy = target_data
  low_pos = which(dummy < 41.33333) #lower prob. must update for every env variable
  mid_pos = which(dummy >= 41.33333) #lower prob. must update for everyb env variable
  high_pos = which(dummy > 56.00000) #higher prob. must update for every env variable
