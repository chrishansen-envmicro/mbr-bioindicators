...
indval_filter <- function(in_data, target_data, cutoff = 0.05){
  ...
  dummy = target_data
  low_pos = which(dummy < 31.00000) #lower prob. must update for every env variable
  mid_pos = which(dummy >= 31.00000) #lower prob. must update for everyb env variable
  high_pos = which(dummy > 72.66667) #higher prob. must update for every env variable
...
#STOP! Make sure you are using the correct rarified dataset (MH, GNP, Both)
write.csv(data.filt.rarified.gnp@sam_data, 'meta_18s_gnp.csv')
meta <- read.csv('meta_18s_gnp.csv')
...
ggplot(cov_data_order, aes(x = env_variable, y = r_squared, color = cat)) +
  geom_jitter(alpha = 0.2, width = 0.1, size = 4) +
  ...
  theme(axis.title = element_text(size = 15), #Modify text sizes to improve figure readability
