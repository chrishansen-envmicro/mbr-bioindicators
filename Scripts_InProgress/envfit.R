...
# Adonis test
adonis2(vst_bray_mh_nowm ~ quadrant_ew, data = sampledf_mh_nowm)

# Homogeneity of dispersion test
beta <- betadisper(vst_bray_mh_nowm, sampledf_mh_nowm$quadrant_ew)
permutest(beta)

export <- as.data.frame(sample_data(vst.data.0))
write.csv(export, file = "beta_data.csv")
...
plot_ordination(vst_reps, pca_reps, type="sites", color="sample_name") +
  ...
  coord_fixed((pca_reps_pc2) / pca_reps_pc1) + 
...
groups <- sampledf_reps$sample_name

# Calculate mean within-group (replicate) distances
within_dist <- mean(vst_reps_dist[groups == groups])  

# Calculate mean between-group distances
between_dist <- mean(vst_reps_dist[groups != groups]) 
...
within_dist_list <- lapply(unique(sampledf_reps$sample_name), function(site) {
  samples <- sampledf_reps$ncbi_name_16s[sampledf_reps$sample_name == site]
  if (length(samples) >= 2) {  # Only sites with replicates
    site_dist <- as.matrix(vst_reps_dist)[samples, samples]
...
set.seed(503)
vars_to_remove <- c("K", "SPC", "HCO3","CaCO3")
meta_filtered <- meta_table_transform[, !colnames(meta_table_transform) %in% vars_to_remove]

env_fit <- envfit(pca_all, env = meta_filtered,
...
# Factor group labels (e.g., "Mount Hood", "Glacier National Park")
group_labels <- as.factor(vst.data.0@sam_data$location)

# Define solid colors for each group (you can use palette colors or custom)
base_colors <- c("Mount Hood" = "#b3692b", "Glacier National Park" = "#667c74")

# Apply alpha to colors using adjustcolor
transparent_colors <- adjustcolor(base_colors[group_labels], alpha.f = 0.9)


palette(c(adjustcolor("#667c74"), adjustcolor("#b3692b")))
p <- ordiplot(pca_all, display = "sites", type = "n", 
              xlab = "PC1 (13.3%)", ylab = "PC2 (10.4%)", # informed by exploratory PCA (line 140)
              xlim=c(-10,10), ylim=c(-15,15), 
              xaxs = "i", yaxs = "i", cex.lab = 1.5)
plot(env_fit_adj, p = 0.05, add = TRUE, col = adjustcolor("black"))
...
points(pca_all, display = "sites", pch = c(16,17)[as.factor(vst.data.0@sam_data$location)],
       col = as.factor(vst.data.0@sam_data$location), cex = 1)
...
#w/o wm
plot_ordination(vst_mh_nowm, pca_mh_nowm, type="sites", color="quadrant_ew")
...
points(pca_mh, display = "sites",
       col = as.factor(vst_mh@sam_data$quadrant), pch = 16, cex = 2)
ordiellipse(p_mh, vst_mh@sam_data$quadrant, kind = "sd", conf=0.95, 
...
p_mh_nowm <- ordiplot(pca_mh_nowm, display = "sites", type = "n", 
                      xlab = "PC1 [14.7%]", ylab = "PC2 [10.3%]",
                      ylim = c(-12,14))
plot(env_fit_adj_mh_nowm, p = 0.05, add = TRUE, col = "slategrey")
points(pca_mh_nowm, display = "sites",
       col = as.factor(vst_mh_nowm@sam_data$quadrant_ew), pch = 16, cex = 2)
ordiellipse(p_mh_nowm, vst_mh_nowm@sam_data$quadrant_ew, kind = "sd", conf=0.95, 
            col = c(adjustcolor("#FE938C", alpha.f=0.6), 
                    adjustcolor("#4281A4", alpha.f = 0.6),
                    adjustcolor("#4281A4", alpha.f = 0.6),
                    adjustcolor("#50514F", alpha.f = 0.6),
                    adjustcolor("#70C1B3", alpha.f = 0.6)),
            lwd=3)
legend(x = 14, y = 14, inset = 0.02, pch = 16, col=unique(as.factor(vst_mh_nowm@sam_data$quadrant_ew)), 
       legend = unique(as.factor(vst_mh_nowm@sam_data$quadrant_ew)), box.lty = 0, cex = 1.3)
