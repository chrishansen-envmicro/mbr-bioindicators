...
quantile(environmental_data$dist_from_summit, c(0:3/3))
environmental_data_2 <- filter(environmental_data, tds != "NA")
...
indval_filter <- function(in_data, target_data, cutoff = 0.05){
  ...
  dummy = target_data
  low_pos = which(dummy < 5402.167) #lower tertile. MUST update for every env variable
  mid_pos = which(dummy >= 5402.167) #middle tertile. MUST update for every env variable
  high_pos = which(dummy > 9312.200) #higher tertile. MUST update for every env variable
...
#STOP! Make sure you are using the correct rarified dataset (MH, GNP, Both)
sam_dat <- as.matrix(sample_data(data.filt.rarified.mh))
...
tax_data <- otu_final[,69:16562] #select just the ASV/OTU table part of the file (you may have to scroll to the back of the OTU file to find it...)
...
modelfolder <- "~/Desktop/Hydro/mod"
predfolder <- "~/Desktop/Hydro/pred"
evalfolder <- "~/Desktop/Hydro/eval1"
...
train_covariation_framework(in_dataset = tax_data, out_dataset = environmental_data, 
                            target_feature = "dist_from_summit", model = "rf", 
...
modelfolder <- "~/Desktop/Hydro/mod"
predfolder <- "~/Desktop/Hydro/pred"
evalfolder <- "~/Desktop/Hydro/eval2"
...
modelfolder <- "~/Desktop/Hydro/mod"
predfolder <- "~/Desktop/Hydro/pred"
evalfolder <- "~/Desktop/Hydro/eval3"
...
cov_data <- read.csv("cov_data_5.27.csv") %>%
