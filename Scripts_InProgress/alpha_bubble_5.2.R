...
# total - no rare
rel_abund_norarefied <- data_filt_clean %>% transform_sample_counts(function(x) {
  x/sum(x)
})


# MH only - no rare
data_filt_clean_mh <- subset_samples(data_filt_clean, location == "Mount Hood")
rel_abund_norarefied_mh <- data_filt_clean_mh %>% transform_sample_counts(function(x) {
  x/sum(x)
})

# GNP only - no rare 
data_filt_clean_gnp <- subset_samples(data_filt_clean, location == "Glacier National Park")
...
# both systems
ps_order <- phyloseq::tax_glom(rel_abund_rarefied_mh, "Genus")
ps_order_edit <- subset_taxa(ps_order, Phylum != "uncultured" & Class != "uncultured" & Order != "uncultured" &
                               Family != "uncultured" & Genus != "uncultured" & Genus != "Unknown_Genus" & Genus != "uncultured_ge")
...
# MH
ps_order <- phyloseq::tax_glom(rel_abund_norarefied_mh, "Class")
...
sum(phylum_abundance$mean_abundance[1:11])
