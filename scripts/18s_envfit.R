...
map_filt <- filter(map, bio_rep != "blank")
map_filt <- filter(map, ncbi_name_18s != "")
...
filter.data <- filter_taxa(data_spst, function(x) sum(x > 0) > (0.1 * length(x)), TRUE)

#develop rarefaction curves and visualize distribution of read lengths: OPTIONAL
tab <- otu_table(filter.data)
...
data.filt.clean <- subset_samples(filter.data, sample_sums(filter.data) >= (10000))
...
##it works! need to understand the reasoning behind this fix however.
...
# Adonis test
adonis(vst_bray ~ location, data = sampledf)
