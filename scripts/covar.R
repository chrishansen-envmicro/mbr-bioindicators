...
#below removes singletons, exclusively
...
rare <- rarecurve(otu_tab, step=100, ylab="OTU",  label=F)
#develop rarefaction curves and visualize distribution of read lengths: OPTIONAL
#rarecurve(t(otu_table(data_filt)), step = 100, label = F)
abline(v=8000, lw = 1) 

#identify which samples had fewer than 10^3 reads and remove (12)
data.filt.clean <- subset_samples(filter.data, sample_sums(filter.data) >= (8000))
...
#complete rarefaction - reflects 
data.filt.rarified = rarefy_even_depth(data.filt.clean, rngseed=1, 
                                       sample.size=0.9*min(sample_sums(data.filt.clean)), 
                                       replace=F) %>%
  
  # remove NAs for each subsample when completing the machine-learning framework (example: pH; below)
  subset_samples(sodium != "NA")
...
indval_filter <- function(in_data, target_data, cutoff = 0.05){
  ...
  dummy = target_data
  low_pos = which(dummy < 0.2200000) #lower tertile. MUST update for every env variable
  mid_pos = which(dummy >= 0.2200000) #middle tertile. MUST update for every env variable
  high_pos = which(dummy > 0.3333333) #higher tertile. MUST update for every env variable
...
otu_flip_num$ncbi_name_16s<-row.names(otu_flip) # places row names as sample ID column
...
otu_final<-left_join(meta, otu_flip_num , by = c("ncbi_name_16s" = "ncbi_name_16s")) # join based on sample IDs, assuming they're the same for both OTU table and metadata
tax_data <- otu_final[,69:16316] #select just the ASV/OTU table part of the file (you may have to scroll to the back of the OTU file to find it...)
...
modelfolder <- "/Users/chrishansen/Desktop/mbs/hydro_paper/mod"
predfolder <- "/Users/chrishansen/Desktop/mbs/hydro_paper/pred" 
evalfolder <- "/Users/chrishansen/Desktop/mbs/hydro_paper/eval1"
...
modelfolder <- "/Users/chrishansen/Desktop/mbs/hydro_paper/mod"
predfolder <- "/Users/chrishansen/Desktop/mbs/hydro_paper/pred" 
evalfolder <- "/Users/chrishansen/Desktop/mbs/hydro_paper/eval2"

##num_folds = 10 for 10-fold cross-validation; this should be the number of 
#   samples for leave-one-out cross-validation
train_covariation_framework(in_dataset = tax_data, out_dataset = environmental_data, 
                            target_feature = "sulfate", model = "rf", 
                            model_target = modelfolder, prediction_target = predfolder, 
                            evaluation_target = evalfolder, num_folds = 10,
                            feature_selection_method = "indVal")
### REP 3
...
modelfolder <- "/Users/chrishansen/Desktop/mbs/hydro_paper/mod"
predfolder <- "/Users/chrishansen/Desktop/mbs/hydro_paper/pred" 
evalfolder <- "/Users/chrishansen/Desktop/mbs/hydro_paper/eval3"
...
cov_data <- read.csv("cov_data.csv") %>%
...
cov_data_order <- cov_data %>% # reorder variables to reflect an increasing mean r_squared
  mutate(env_variable = fct_reorder(env_variable, r_squared_orig, mean, na.rm = TRUE,
                                    .desc = TRUE))
# plot
